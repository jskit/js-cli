...
const { defaultPreset } = require('@jskit/cli/lib/options')
...
test('build as lib', async () => {
  ...
  const { stdout } = await project.run('js-cli-service build --target lib --name testLib src/components/HelloWorld.vue')
...
test('build as lib (js)', async () => {
  ...
  await project.write('src/main.js', `
    ...
    export const bar = 2
  `)
  const { stdout } = await project.run('js-cli-service build --target lib --name testLib src/main.js')
  expect(stdout).toMatch('Build complete.')
