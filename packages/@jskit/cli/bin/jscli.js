...
if (
  slash(process.cwd()).indexOf('/packages/test') > 0 && (
    fs.existsSync(path.resolve(process.cwd(), '../@jskit')) ||
    fs.existsSync(path.resolve(process.cwd(), '../../@jskit'))
  )
) {
...
program
  ...
  .description('create a new project powered by js-cli-service')
...
program
  ...
  .description('inspect the webpack config in a project with js-cli-service')
  .action((paths, cmd) => {
...
program
  ...
  .description('generate a project from a remote template (legacy API, requires @jskit/cli-init)')
  .option('-c, --clone', 'Use git clone when fetching remote template')
  .option('--offline', 'Use cached template')
  .action(() => {
    loadCommand('init', '@jskit/cli-init')
  })
